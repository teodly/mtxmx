...
impl<T: Default> Matrix<T> {
    pub fn new(outputs_count: usize, inputs_count: usize, create_item_cb: impl Fn() -> T) -> Self {
        Self {
            flat: (0..outputs_count * inputs_count)
                .map(|_| create_item_cb())
                .collect(),
            outputs_count,
            inputs_count,
        }
...
fn print_matrix(matrix: &Matrix<AtomicF32>) {
    ...
    for out_index in 0..matrix.outputs_count {
        let line = (0..matrix.inputs_count)
            .map(|in_index| {
...
impl RealTimeMixer {
    ...
    fn process(
        &self,
        internal: &mut RealTimeInternal,
        _client: &jack::Client,
        ps: &jack::ProcessScope,
    ) {
        for (output_index, (output_port, is_active)) in internal
            .output_ports
            .borrow_mut()
            .iter_mut()
            .zip_eq(&self.active_outputs)
            .enumerate()
        {
            ...
            for (input_index, input_port) in internal
                .input_ports
                .iter()
                .zip_eq(&self.active_inputs)
                .enumerate()
                .filter(|(_, (_, a))| a.load(Ordering::Relaxed))
                .map(|(i, (p, _))| (i, p))
            {
        ...
        for (input_index, input_port) in internal
            ...
            .map(|(i, (p, _))| (i, p))
        {
            self.input_meters[input_index].fetch_max(
                input_port
                    .as_slice(ps)
                    .iter()
                    .map(|v| v.abs())
...
impl<N, P> HighLevelMixer<N, P> {
    ...
    fn commit_point(&self, out_index: usize, in_index: usize) {
        ...
        if output.rt_channels.len() == input.rt_channels.len() {
            ...
            for (rt_out_index, rt_in_index) in
                itertools::zip_eq(&output.rt_channels, &input.rt_channels)
            {
    ...
    fn disable_output(&mut self, index: usize, clear_matrix: bool) {
        for &chi in &self.outputs[index].rt_channels {
            if clear_matrix {
                self.rt
                    .matrix
                    .for_each_input(chi, |point| point.store(0.0, Ordering::Relaxed));
            }
            for conn in self.rt_outputs[chi].get_connections() {
                let _ = self
                    .jack_client
                    .as_client()
                    .disconnect_ports_by_name(&self.rt_outputs[chi].name().unwrap(), &conn);
            }
    ...
    fn disable_input(&mut self, index: usize, clear_matrix: bool) {
        for &chi in &self.inputs[index].rt_channels {
            if clear_matrix {
                self.rt
                    .matrix
                    .for_each_output(chi, |point| point.store(0.0, Ordering::Relaxed));
            }
            for conn in self.rt_inputs[chi].get_connections() {
                let _ = self
                    .jack_client
                    .as_client()
                    .disconnect_ports_by_name(&conn, &self.rt_inputs[chi].name().unwrap());
            }
    ...
    fn update_output(&mut self, index: usize) {
        ...
        for (ch_index, (connect_to, &rt_index)) in output
            .connect_to
            .iter()
            .zip_eq(output.rt_channels.iter())
            .enumerate()
        {
            ...
            for pn in connect_to {
                let _ = self
                    .jack_client
                    .as_client()
                    .connect_ports_by_name(&self.rt_outputs[rt_index].name().unwrap(), pn);
            }
        }
        let s =
            serde_json::to_string(&get_connections_of_endpoint(output, &self.rt_outputs)).unwrap();
        let _ = self.to_mqtt.try_send(ToMQTT {
            topic: format!("status/out{:03}/connected_to", index + 1),
            value: s.into(),
            important: true,
        });
    }
    fn update_input(&mut self, index: usize) {
        ...
        for (ch_index, (connect_to, &rt_index)) in input
            .connect_to
            .iter()
            .zip_eq(input.rt_channels.iter())
            .enumerate()
        {
            ...
            for pn in connect_to {
                let _ = self
                    .jack_client
                    .as_client()
                    .connect_ports_by_name(pn, &self.rt_inputs[rt_index].name().unwrap());
            }
        }
        let s =
            serde_json::to_string(&get_connections_of_endpoint(input, &self.rt_inputs)).unwrap();
        let _ = self.to_mqtt.try_send(ToMQTT {
            topic: format!("status/in{:03}/connected_to", index + 1),
            value: s.into(),
            important: true,
        });
    }
    fn enable_output(&mut self, index: usize) {
        ...
        for _ in self.outputs[index].connect_to.iter() {
            if let Some(rt_index) = self
                .rt
                .active_outputs
                .iter()
                .position(|a| !a.load(Ordering::Relaxed))
            {
...
fn get_connections_of_endpoint(
    endpoint: &Endpoint,
    rt_ports: &Vec<jack::Port<jack::Unowned>>,
) -> Vec<Vec<String>> {
    endpoint
        .rt_channels
        .iter()
        .map(|&chi| rt_ports[chi].get_connections())
        .collect_vec()
}
...
impl jack::NotificationHandler for JackNotifications {
    ...
    fn sample_rate(&mut self, _: &jack::Client, srate: jack::Frames) -> jack::Control {
        ...
        let _ = self.to_mqtt.try_send(ToMQTT {
            topic: "status/sample_rate".to_owned(),
            value: srate.to_string(),
            important: true,
        });
    ...
    fn xrun(&mut self, _: &jack::Client) -> jack::Control {
        ...
        self.xrun_counter += 1;
        let _ = self.to_mqtt.try_send(ToMQTT {
            topic: "status/xruns".to_owned(),
            value: self.xrun_counter.to_string(),
            important: true,
        });
        jack::Control::Continue
    }
...
async fn main() {
    ...
    control_client
        .subscribe(args.root.clone() + "/#", QoS::AtLeastOnce)
        .await
        .expect("failed to subscribe");
    let prefix = if args.root.is_empty() {
        "".to_owned()
    } else {
        args.root + "/"
    };
    ...
    let rt_mixer = Arc::new(RealTimeMixer {
        ...
        active_outputs: (0..args.output_channels_max)
            .map(|_| false.into())
            .collect(),
        active_inputs: (0..args.input_channels_max).map(|_| false.into()).collect(),
        output_limiters: (0..args.output_channels_max)
            .map(|_| Limiter {
                threshold: 1.0.into(),
    ...
    let mut rt_internal = RealTimeInternal {
        output_ports: RefCell::new(
            (0..args.output_channels_max)
                .map(|i| {
                    jack_client
                        .register_port(&format!("raw_out{i:04}"), jack::AudioOut::default())
                        .unwrap()
                })
                .collect(),
        ),
        input_ports: (0..args.input_channels_max)
            .map(|i| {
                jack_client
                    .register_port(&format!("raw_in{i:04}"), jack::AudioIn::default())
                    .unwrap()
            })
            .collect(),
    };
    let rt_outputs: Vec<_> = rt_internal
        .output_ports
        .get_mut()
        .iter()
        .map(|port| port.clone_unowned())
        .collect();
    let rt_inputs: Vec<_> = rt_internal
        .input_ports
        .iter()
        .map(|port| port.clone_unowned())
        .collect();
    ...
    let active_client = jack_client
        .activate_async(
            JackNotifications {
                ...
                rescan: rescan_counter.clone(),
                report_connections: report_connections_counter.clone(),
            },
            jack_process,
        )
        .unwrap();
    ...
    let mut mixer = HighLevelMixer {
        ...
        outputs: (0..args.output_endpoints_max)
            .map(|i| Endpoint {
                name: format!("out{:03}", i + 1),
                gain: 1.0,
                enabled: true,
                connect_to: vec![],
                rt_channels: vec![],
            ...
            tokio::runtime::Builder::new_current_thread()
                ...
                .block_on(async {
                    loop {
                        ...
                        if let Some(pkt) = pkt {
                            let _ = control_client1
                                .publish_bytes(
                                    prefix1.clone() + &pkt.topic,
                                    if pkt.important {
                                        QoS::AtLeastOnce
                                    } else {
                                        QoS::AtMostOnce
    ...
    to_mqtt_sender
        .send(ToMQTT {
            topic: "status/sample_rate".into(),
            value: mixer.jack_client.as_client().sample_rate().to_string(),
            important: true,
        })
        .await
        .unwrap();
